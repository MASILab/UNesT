function orig_inv_seg_file = postproc_pipeline(target_fname,working_dir,img_dir,result_dir, in)

% 
% testing_T1_root_dir = [working_dir filesep 'deep_learning'];
% working_root_dir = [working_dir filesep 'all_piece'];
mni_result_dir = result_dir;
% 
% 
% % method = 'cluster';
% method = 'single';
...
% 
% 
% sublist = dir([mni_result_dir filesep '*.nii.gz']);
% 
...
%         toc;
        fprintf('start inv reg');
        
        %orig space
        T1_orig_file = target_fname;
        
        %MNI 
%         T1_MNI = [img_dir filesep  sprintf('%s.nii.gz',subName)];
        
        mni_fname = [get_basename(target_fname)  '_target_processed.nii.gz'];
        T1_MNI = [img_dir filesep mni_fname];
        final_Seg = [mni_result_dir filesep mni_fname];
%         Manual_orig_seg = [orig_seg_root_dir filesep sprintf('%s_glm.nii.gz',subName)];
        
        dice_orig_mat_dir = [working_dir filesep 'dice_orig_mat'];
        if ~isdir(dice_orig_mat_dir);mkdir(dice_orig_mat_dir);end;
        
        orig_invreg_dir = [working_dir filesep 'inv_orig_space'];
        if ~isdir(orig_invreg_dir);mkdir(orig_invreg_dir);end;
        orig_inv_seg_file = [working_dir filesep sprintf('%s_orig_seg.nii.gz',get_basename(target_fname))];
        
        inv_reg_working_dir = orig_invreg_dir;
...
%         toc;
        ...
        fprintf('inverse transform done %s\n',get_basename(target_fname));
